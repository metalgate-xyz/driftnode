...
- Two keypairs exist per identity, serving different purposes: the Ed25519 keypair (§7) signs application events and *is* the identity; a separate WireGuard (X25519) keypair, generated by Tailcat, authenticates the transport tunnel. This mirrors how Signal-style protocols separate an identity key from a session/transport key, and keeps the transport swappable without touching identity. The WireGuard keypair is persisted in the local store by default, so the node's address token stays stable across restarts and followers can keep dialing it without a lookup. The `--ephemeral` flag opts out for throwaway sessions where a fresh key each run is acceptable; the key lives in the db, not a file.
...
1. A bootstrap/seed entry needs a **long-lived Tailcat keypair** so its published token stays valid across restarts. The default store-persisted key already provides this; a seed entry additionally keeps the same key when moving hosts by restoring it from backup (§8), not by mounting a key file.
2. Listing is an explicit action — `driftnode relay enable` publishes the node's token into the DHT as a bootstrap/relay candidate. Nothing does this automatically.
3. `driftnode relay status` reports current state: not advertised, or advertised as a bootstrap candidate.
...
- Native zens persist their Tailcat keypair in the local store by default (§5.1), so a restart reuses the same token and followers' existing routing binding keeps working without any lookup. The identity string never encodes routing, but the token is stable for as long as the key is.
- A zen that moves to a new device restores the transport key from backup (§8), preserving the same token across the move. If the key is lost (no backup), the zen reappears under a new token and must be re-discovered through the normal mechanisms below. An explicit `driftnode rotate-key` does the same on purpose: it generates a fresh key, persists it, and restarts the listener under the new token.
- The DHT (`go-libp2p`'s `kad-dht`), when integrated, stores a **signed provider record** per pubkey for the case the token does change: the identity's owner, or whichever native zen is currently relaying for them, publishes their current routing descriptor — a Tailcat token for a native zen, or a native zen's token for a relayed browser user — signed by the pubkey's own key so a relay cannot spoof the record.
...
- Records carry a short TTL and are refreshed periodically, so a token that does change (lost key, explicit rotation, relayed browser user) produces a new record while the identity string itself never changes.
...
- **Export:** serializes the account-critical state — the keypair (encrypted with the user's passphrase), the user's own Profile log, Detail log, own PostLog, their local petname map (§9.4), and the tailcat transport key (encrypted with the same passphrase, so the address token survives a device restore) — into a single CBOR file. In the browser this triggers a native download via the `Blob`/anchor-download APIs (`syscall/js`); on native, `driftnode backup export` writes it directly. Only the keypair and transport key are encrypted; the logs are already public, signed content, so re-encrypting them adds no protection while making the file harder to inspect for debugging.
...
| `driftnode rotate-key` | Generate a fresh tailcat transport key, persist it, and restart the listener under the new address token (§7.5) |
