...
**Goals**
- No company-owned backend: no accounts database, no auth server, no central feed server.
- Two client artifacts, one language, no other application languages and no hand-written JavaScript: a **browser app compiled from Go to WebAssembly** and a **native peer binary** (`driftnode`, also Go) that runs identically on any machine — VPS, Raspberry Pi, laptop, desktop.
...
- **Zero-infrastructure discovery between total strangers is not achievable.** Two peers with no prior relationship cannot find each other on the open internet without *some* rendezvous point. This is minimized to the smallest possible, swappable, community-runnable component (§6) — never a component that holds user data.
- No guaranteed real-time global delivery. This is a gossip network, not a queue; offline peers catch up when they reconnect.
...
└──────────────────────────────────────────────────────────────────────────┘
                ...
                ▼                                            ▼
        Peer browser (same stack)              driftnode (native, Go, §5)
                                                same shared core module +
                                                 • pion/webrtc (native build) → browser peers
...
Everything in this diagram is peer-owned. The only piece that isn't literally "someone's browser or someone's binary" is a thin, replaceable, signed static file (`bootstrap.yaml`, §6.1) used to find the *first* peers — never a stateful service, never a data holder.

Browsers can only dial out over WebRTC; WASM has no viable `libp2p` or Tailcat transport today. `driftnode` therefore runs two connection-establishment mechanisms — `pion/webrtc` to terminate browser connections, Tailcat to reach other native peers through NAT (§5.1) — plus `go-libp2p`'s `kad-dht` purely for DHT-based peer discovery (finding *which* peer to reach, independent of *how*). All of this is bridged at the application layer, not the transport layer: every path feeds the same event-log sync protocol (§7.3), so which mechanism delivered a given piece of gossip never leaks into the data model. Because both the browser and native builds compile the same shared Go module (§3), that sync protocol is one implementation, not two that need to agree.
...
| DHT / peer discovery | [`go-libp2p`](https://github.com/libp2p/go-libp2p) `kad-dht` | The original, most mature `libp2p` implementation; used purely for discovery, independent of the transport that ultimately carries data |
...
Content-addressed media is chunked and streamed as just another message type over whichever connection is already established for event-log sync (§7.3) — a Tailcat tunnel between native peers, or a WebRTC data channel for browsers — rather than a separate, dedicated blob-transfer library.
...
Browsers can dial out but cannot accept inbound connections. That constraint is deliberate for casual users — zero attack surface, no port-forwarding, nothing listening on their machine — but it means some jobs require a peer that *can* accept inbound connections. That role is filled by a single artifact, `driftnode`, not a deployment tier: it is the same binary and the same behavior whether it runs on a VPS, a Raspberry Pi, or a laptop.

**These additional jobs specifically require a *reachable* peer:**

| Job | Why it needs a reachable peer |
|---|---|
...
| Blob hosting for others | Serving media to peers who were offline when it was originally posted, over the same connection mechanism used for event-log sync (§4) |
...
- **Trust legibility:** the UI distinguishes peers by connection quality, not by hardware class — a peer reached via a direct hole-punched tunnel is shown as such; one reached only via DERP relay is shown as relayed. This is what the TUI's peers panel (§12.3) reflects.
- **The WebRTC signaling relay (§4) is a separate, optional companion process** — run alongside `driftnode` only when its operator chooses to offer browser-discovery signaling. `bootstrap.yaml` lists these under `seed_relays`, distinct from `seed_peers` (§6.1), because a node can run one without the other.
...
**What this replaces, precisely:** the *point-to-point connection establishment and NAT traversal* between two native peers who already know each other's identity. `go-libp2p`'s `kad-dht` remains the discovery mechanism (finding *which* peer to dial); Tailcat only replaces the *how* of dialing once a target is known. This distinction matters because Tailcat's own model — one listener, one connector, an out-of-band token — is single-connection, netcat-shaped, not a discovery or multi-peer mesh protocol.

**How it fits the design:**
- Each `driftnode` runs a Tailcat listener, producing a token (`tc1q...`) that encodes a WireGuard public key and a DERP region — a *routing descriptor*, not an identity. It is published as the peer's reachability record (§7.5) in place of a `libp2p` multiaddr.
- To sync with a known peer, `driftnode` resolves their current token via the DHT and connects to it directly through the Tailcat library. The resulting encrypted duplex stream carries the event-log sync protocol described in §7.3 — Tailcat is a byte-stream transport underneath that protocol, not a replacement for it.
...
Two peers with no shared history cannot open a direct connection without some way to exchange addresses. Three mechanisms, in order of how much they depend on anyone but the two peers involved:
...
3. **DHT bootstrap** — native peers use `go-libp2p`'s `kad-dht`, seeded from a small published list of bootstrap addresses (the same pattern as BitTorrent or IPFS). This is also where a browser client gets its first list of native peers to try.
...
seed_peers:
  - token: "tc1qabc...xyz"   # long-lived Tailcat token for a seed native peer, §5.1/§5.2
    kind: native_peer
crawl_seeds:
...
- **Degrades gracefully:** fetch order is (1) a cached copy from the last successful run, (2) a network fetch from any configured mirror, (3) a small hardcoded fallback list compiled into the binary. Once connected, peers exchange their own routing tables via Kademlia, so `bootstrap.yaml` is a cold-start convenience, not an ongoing dependency.
- **Publication:** the project ships an initial signed file pointing at its own seed peers and relays. Anyone can fork it, sign their own copy with their own key, and point a build at a different file — the same trust model as a Linux distribution's package-signing key.
...
1. Maintainers stand up the initial seed native peers and DERP relays (§13, several regions).
...
4. They sign `bootstrap.yaml` v1 with `seed_peers`, `derp_relays` pointing at their own infrastructure, and `crawl_seeds` set to their own pubkeys — the entire genesis dataset.

**As external users join:**
5. New clients fetch and verify `bootstrap.yaml`, dial `seed_peers`, and run the built-in crawl (§9.3) starting from `crawl_seeds` — at this point that crawl can only discover the maintainers and whoever has followed them so far. Search results and any "suggested accounts" surface are transparently just the maintainers at this stage; the UI states this rather than obscuring it.
...
9. Once a node has crawled once, `crawl_seeds` stops mattering to it — the same cold-start-only property as the peer bootstrap list (§6.1). It only ever affects a brand-new user's first session.
...
- **Identity string:** `driftnode:<base32-pubkey>` — the entire address. No hostname, no relay, no routing detail is ever encoded in it, because routing information changes (an operator switches native peer, a relay goes down, a device moves networks) while identity must not.
- **Signing:** every event is signed with its author's private key before being added to their log, so peers can verify authenticity independent of how they received it — browser, relay, or otherwise. Only events signed by an identity's own key are ever accepted into that identity's log, which is what prevents anyone else from forging posts, follows, or deletes regardless of which peer relayed them.
- **No CRDT engine, by design.** Each identity's log has exactly one legitimate writer — its own signing key (nobody else can produce a validly-signed event for it) — so there is never a concurrent-write conflict between two authors to reconcile. "Merging" two peers' views of a log is set union over immutable, individually-signed events, not CRDT-style conflict resolution. This is the same model Nostr and Secure Scuttlebutt use.
...
- **Media:** attachments are content-addressed (BLAKE3 hash) and fetched lazily as chunked transfers over whichever connection is already established for event-log sync — a Tailcat tunnel between native peers, or a WebRTC data channel between browsers (§4) — never inlined into an event, keeping logs light regardless of media volume.
...
| **PostLog** | `Post`, `Reply`, `Like`, `Delete` events — the content stream, potentially large and long-lived | Only peers who follow this identity (§9.1) |

Because each log is just a named, independently-requestable stream of signed events, a peer can ask for "give me `driftnode:<pubkey>`'s Profile log" without ever touching their PostLog. This is what makes the crawler's low cost an architectural property rather than a policy: a crawl of thousands of accounts never requests, and therefore never receives, anyone's post history.

**Current state is a deterministic projection over the log**, computed independently by every peer from whatever subset of events it has: the current profile is the latest `Profile` event by timestamp; the current follow set is the result of replaying `Follow`/`Unfollow` events in order; the current post list is every non-tombstoned `Post`/`Reply` event. Because projection is a pure function of the event set, two peers with different partial views only ever differ in *completeness*, never in *disagreement* about what a given event means.
...
**Likes and counts are observational, not authoritative.** Because a `Like` lives in the liker's own log — nobody can write into someone else's log without a valid signature from that identity's own key — a "like count" on a post is always "the number of `Like` events this particular peer has observed referencing this ID," never a true global count. The same caveat applies to follow-counts surfaced by the crawler (§9.3).

**`Delete` is a tombstone, not erasure.** It is itself a signed event referencing the target's ID. Peers who already replicated the original content before a delete propagated may still hold a copy — a structural property of gossip replication.

### 7.3 Wire Protocol

Synchronizing a log between two peers is a set-reconciliation problem, not a CRDT merge: each side needs to learn which signed events the other has that it doesn't, and transfer only those. The baseline mechanism — sufficient for Phase 0 (§13) and revisited only if it proves too coarse in practice — is a timestamp cursor: "send me every event in log X after time T," with the requester's last-synced cursor persisted locally per peer per log. A more precise set-reconciliation scheme (e.g. exchanging sorted event-ID ranges or a compact set sketch) is worth evaluating once real usage patterns exist, but is not a blocker for an initial correct implementation. Messages are framed as length-prefixed CBOR and sent over whichever byte stream the connection-establishment mechanism provides — a Tailcat tunnel between native peers (§5.1), or a `pion/webrtc` data channel for browser peers (§4). Because the sync protocol only depends on a byte-stream abstraction and both ends run the same shared implementation, bridging these different mechanisms on `driftnode` (§2, §5) requires no protocol translation.
...
|---|---|
| PostLogs of followed accounts | Retained fully by default. Optional `postlog_retention_days` config caps local history for operators who want it — pruned locally only, never removed network-wide, since peers who still hold it can still serve it |
...
These are `driftnode config` knobs (§12.2) rather than architectural exceptions — a personal node and a heavily-peered public relay set them differently, using the same code path.
...
- The DHT (`go-libp2p`'s `kad-dht`) stores a **signed provider record** per pubkey: the identity's owner, or whichever native peer is currently relaying for them, publishes their current routing descriptor — a Tailcat token for a native peer, or a native peer's token for a relayed browser user — signed by the pubkey's own key so a relay cannot spoof the record.
- Native peers publish their own current Tailcat token (§5.1).
- Browser-only users have their currently-connected native peer publish a record on their behalf — still signed by the user's own key, with the native peer only relaying the publish operation — pointing at that native peer's Tailcat token as the current mirror path.
- Records carry a short TTL and are refreshed periodically — Tailcat tokens for ephemeral (non-seed) peers can legitimately change on every restart — so switching relays, restarting, or changing networks produces a new record while the identity string itself never changes.
- This is the same primitive IPFS calls a provider record and `go-libp2p` calls a peer record, applied to user identities instead of content hashes; only the payload (a Tailcat token instead of a multiaddr) differs from the general pattern.
...
- A node with more uptime and storage accumulates a bigger, more useful crawl cache purely as a side effect of participating longer — the same way a well-seeded BitTorrent peer ends up caching more of the swarm. No node is assigned the job of indexing.
- Browser clients do not crawl themselves — no background compute or storage budget for it — and instead query whichever native peer(s) they are connected to, receiving answers from that peer's local crawl cache.
- "Follow-count" surfaced in search results (§9.4) is a given peer's *observed* count from its own crawl, not a global truth — the same caveat as the Like-count observation in §7.2.
...
2. The client asks its connected native peer(s) to search their local crawl cache of Profile logs — no special server, only whichever peers are already connected.
3. Results show display name, pubkey, an identicon, and light social proof ("followed by 2 people you follow"), explicitly labeled by which peer's crawl cache produced them.
4. Selecting a result resolves that exact pubkey's current address via DHT lookup (§7.5), dials directly, and syncs its Profile log (and PostLog, if the user chooses to follow). The peer that helped find the result drops out of the trust path immediately — it was used only for discovery, never for verification.
...
- Mute/block lists are client-side and themselves shareable as signed event logs — subscribing to a friend's or a community's blocklist, the same pattern Bluesky uses for labelers, but peer-published rather than server-published.
...
| Search is always best-effort and peer-sourced, never authoritative | Zooko's triangle rules out a central index without reintroducing a central authority; built-in crawling (§9.3) at least makes this cheap and role-free |
| Like counts, follow counts, and similar aggregates are observational, not global truths | Each peer only knows what it has synced or crawled (§7.2, §9.3); no component is positioned to compute a true global count without becoming a central index |
...
| Native peer nodes are a required part of the network, not optional polish | Browsers structurally cannot accept inbound connections — accepted as the cost of casual users having zero attack surface (§5) |
...
| Tailcat's connection model is single listener/single connector, not natively multi-peer | Needs direct field validation (§13) of whether one listener handles many concurrent peers or whether `driftnode` needs its own multiplexing on top |
| Native peers behind consumer NAT still occasionally can't be reached even via DERP | Rare in practice given relay-of-last-resort, but not eliminated; the node still works fully for its own operator regardless (§5.1) |
...
A single long-running daemon holds the network state — active Tailcat tunnels to native peers (§5.1), `go-libp2p`'s `kad-dht` routing table, gossip subscriptions. The CLI and TUI are both thin clients of that daemon over a local control socket, rather than each invocation opening its own connections from cold.
...
| `driftnode peers list` | Show currently connected peers, their kind (native/relay/browser), and latency |
| `driftnode peers add <token>` | Manually connect to a peer's Tailcat token — useful for direct invite or field testing |
...
| `driftnode bootstrap fetch [--url <mirror>]` | Fetch and verify `bootstrap.yaml`, merging it into the known-peers cache |
...
A `bubbletea` app (styled with `lipgloss`) with four panels, all fed by the same control-socket subscription — the daemon pushes sync and peer-connection events, and the TUI does not poll:

```
┌─ driftnode · you: a3f9…21c4 ─────────────────────────────┬─ Peers (3) ─────────┐
│                                                          │ ● seed-eu (native)  │
...
│ 12:04:05 INFO  kad-dht: routing table now has 7 peers                        │
└───────────────────────────────────────────────────────────────────────────────┘
```

- **Feed panel:** live-updating, merge-sorted timeline; scrollable.
- **Peers panel:** connection kind (native, browser-relayed, direct WebRTC), latency, last-sync time — the trust-legibility distinction from §5.
...
3. Daemon process and control socket (§12.1); direct Tailcat integration (§5.1) between two native nodes on separate machines behind real, different NATs — wiring the CLI's `peers`/`sync` commands to it, proving the event-log sync protocol (§7.3) and set-reconciliation correctness under real latency, packet loss, and restarts. This is also where the open question of concurrent-connection handling (§5.1) gets answered rather than assumed.
...
12. The browser build pointed at the Phase-0 `bootstrap.yaml` and seed nodes — the first time a browser peer joins the live network.
13. Chunked media transfer over existing connections (§4).
14. Moderation lists and pluggable local feed scoring. Tier-2 search (§9.4) is available once Phase 0's crawler and Phase 1's browser client both exist, since no separate index-peer component needs building.
